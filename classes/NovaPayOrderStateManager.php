...
class NovaPayOrderStateManager
{
    ...
    const ORDER_STATES = array(
        ...
        'NOVAPAY_OS_WAITING_FOR_CAPTURE' => array(
            'color' => '#34209E',
            'name' => array(
                'en' => 'Waiting for NovaPay capture',
                'uk' => 'Очікується завершення утриманого платежу NovaPay',
                'ru' => 'Ожидается завершение удержанного платежа NovaPay',
            ),
        ),
    ...
    private function createStateId($state, $color)
    {
        ...
        throw new NovaPayException('Not able to insert the new order state');
    }
    ...
    private function insertNewStateLang($orderStateId, $orderStateName, $languageId)
    {
        ...
        if (!Db::getInstance()->insert('order_state_lang', $data)) {
            throw new NovaPayException('Not able to insert the new order state language');
        }
    }
